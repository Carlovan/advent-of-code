...
impl Hand {
    pub fn parse(cards: &str) -> Hand {
        let card_vals = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J'];
        ...
        let mut counts: Vec<usize> = cards.iter().filter(|&&c| c != 0).counts().into_values().collect();
        let j_count = cards.iter().filter(|&&c| c == 0).count();
        counts.sort();
        if counts.len() == 0 {
            counts.push(0);
        }
